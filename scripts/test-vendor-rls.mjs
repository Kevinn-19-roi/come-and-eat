...
try{
  ...
  const{data:restaurants,error:restaurantError}=await admin.from('restaurants').insert([{name:`Test RLS A ${run}`,slug:'',description:'Test temporaire',validation_status:'approved'},{name:`Test RLS B ${run}`,slug:'',description:'Test temporaire',validation_status:'approved'}]).select('id');if(restaurantError)throw restaurantError;restaurantIds.push(...restaurants.map(row=>row.id));
