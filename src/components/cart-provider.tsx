...
export function CartProvider({children}:{children:React.ReactNode}){
 ...
 const value=useMemo<CartContextValue>(()=>({items,count:items.reduce((s,i)=>s+i.quantity,0),subtotal:items.reduce((s,i)=>s+(i.unitPrice+i.selections.reduce((n,o)=>n+o.price,0))*i.quantity,0),add:(p,selections=[],quantity=1)=>setItems(old=>[...old,{id:uid(),productId:p.id,name:p.name,unitPrice:p.price,quantity,image:p.image?.url,selections}]),remove:(id)=>setItems(old=>old.filter(i=>i.id!==id)),setQuantity:(id,quantity)=>setItems(old=>quantity<1?old.filter(i=>i.id!==id):old.map(i=>i.id===id?{...i,quantity}:i)),clear:()=>setItems([])}),[items]);
