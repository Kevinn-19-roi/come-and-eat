...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description = restaurant.description || `Commandez chez ${restaurant.name} sur Come & Eat.`;
  return { title: restaurant.name, description, alternates: { canonical: `/restaurants/${restaurant.slug}` }, openGraph: { title: `${restaurant.name} — Come & Eat`, description, url: `/restaurants/${restaurant.slug}`, type: 'website', images: [restaurant.coverUrl || restaurant.logoUrl || '/opengraph-image.jpg'] } };
}
...
export default async function RestaurantPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><section className="restaurant-page-head"><div className="restaurant-page-cover">{restaurant.coverUrl ? <Image src={restaurant.coverUrl} alt={`Cuisine de ${restaurant.name}`} fill priority sizes="100vw" /> : <span />}</div><div className="store-container restaurant-page-identity"><span className="restaurant-page-logo">{restaurant.logoUrl ? <Image src={restaurant.logoUrl} alt="" fill sizes="110px" /> : restaurant.name.slice(0, 1)}</span><div><p className="section-kicker">{restaurant.isOfficial ? 'Boutique officielle' : 'Restaurant partenaire'}</p><h1>{restaurant.name}</h1><p>{restaurant.description}</p><div className="restaurant-page-meta"><span className={accepting ? 'open' : ''}>{restaurant.operatingStatus === 'paused' ? 'En pause' : accepting ? 'Ouvert' : 'Fermé'}</span><span>{restaurant.averagePrepMinutes} min</span>{restaurant.commune ? <span>{restaurant.commune}</span> : null}{restaurant.deliveryAvailable ? <span>Livraison</span> : null}{restaurant.pickupAvailable ? <span>Retrait</span> : null}</div></div><div className="detail-actions"><FavoriteButton restaurantId={restaurant.id} initial={favorites.restaurantIds.has(restaurant.id)} /><ShareButton title={`${restaurant.name} — Come & Eat`} text={`Découvrez ${restaurant.name} sur Come & Eat.`} path={`/restaurants/${restaurant.slug}`} /></div></div></section>
