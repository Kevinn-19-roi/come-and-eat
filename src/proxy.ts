...
export async function proxy(request:NextRequest){
  ...
  try{
    if(!supabaseUrl||!supabasePublicKey){console.error('[auth-proxy] configuration_missing');return NextResponse.redirect(new URL(`${loginPath}?error=service`,request.url))}
    ...
    return response;
  }catch(error){console.error('[auth-proxy] unexpected_failure',{name:(error as{name?:string})?.name});return NextResponse.redirect(new URL(`${loginPath}?error=service`,request.url))}
}
