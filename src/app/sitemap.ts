...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticPages = ['', '/restaurants', '/menu', '/about', '/contact'].map(path => ({ url: `${base}${path}`, lastModified: new Date(), changeFrequency: path === '' ? 'daily' as const : 'weekly' as const, priority: path === '' ? 1 : path === '/restaurants' || path === '/menu' ? .9 : .6 }));
